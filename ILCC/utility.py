...
'''
import os
from ILCC.pcd_corners_est import exact_full_marker_data
import numpy as np
from ILCC.pcd_corners_est import generate_grid_coords
...
try:
    ...
    import pickle
import cv2
from ILCC.LM_opt import xyz2angle, voxel2pixel
import transforms3d
...
def vis_segments(ind=1):
    ...
    for seg in seg_list:
        if seg.split(".")[-1] == "txt":
            ...
            points_ls = list()
            jdcs_collection = pickle.load(open(os.path.abspath(seg_folder + seg), 'rb'))
...
def vis_segments_only_chessboard_color(ind=1):
    ...
    chessboard_file_name = \
        pickle.load(open(os.path.join(params['base_dir'], "output/pcd_seg/") + str(ind).zfill(
...
def back_project_pcd(img, pcd_arr, color_arr, r_t, i, hide_occlussion_by_marker):
    ...
    if not hide_occlussion_by_marker:  # whether remove occlussions by the chessboard
        ...
        chessboard_result_file_path = os.path.join(params['base_dir'], "output/pcd_seg/" + str(i).zfill(
            params["file_name_digits"]) + "_pcd_result.pkl")
        chessboard_result_file = pickle.load(open(chessboard_result_file_path, "rb"))
